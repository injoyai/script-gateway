...
- **全局视角审查（强制）**：任何改动在落地前必须从全局视角审视其合理性——是否与现有架构 / 数据流 / 既定约定一致，是否波及其他模块或节点类型，是否引入已废弃机制；单点"能跑"不等于合理，禁止只顾局部而破坏全局一致性；无法判断影响面时，先列出受影响范围再动手，不得盲改
- 后端语言：Go；前端框架：CRA + AntD + React Flow（`@xyflow/react`）
- 后端验证：`go build ./...` 与 `go test ./internal/...` 必须通过
- **后端改动后必须重启生效**：任何 Go 代码改动完成后，必须重新编译（`go build ./...` 通过）并重启后端服务，使最新接口与逻辑对前端生效；编译失败须先修复再重启。禁止“改了后端却不重启”——否则前端调用的仍是旧接口
- 命令操作：禁止 `git add -A`、禁止 `git push --force`、禁止未授权的破坏性 git 操作
- 改动范围：仅做用户明确要求的事，不要顺手"重构""清理""加错误处理""加注释"
- **数据流可视化为唯一配置入口**（替代原“双向同步”约定）：
  - 监听 / 处理 / 转发三段的配置全部在「数据流可视化」（DataFlowCanvas）完成（新建 / 编辑 / 删除 / 启停，含 InlineEditPanel 与 NodeEditModal）
  - 数据管理独立页（data-listener / unified 等侧边栏菜单）将逐步移除，不再要求与画布双向同步
  - 新增 / 修改容器类型或字段时，只需在画布侧（fieldSchema / InlineEditPanel / NodeEditModal）实现，无需再维护独立管理页
  - mock / 占位功能（数据采集 / 数据服务 / 数据解析）不在画布实现，直接清理菜单与路由
  - 共享配置 schema 仍抽取到公共模块（如 `processorSchema.ts`、`fieldSchema.ts`）
